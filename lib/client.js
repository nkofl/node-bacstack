...
class Client extends EventEmitter {
  constructor(options) {
    ...
    this._segmentStore = [];
    this._foreignDeviceTimer = null;

    this._settings = {
      port: options.port || 47808,
      listenPort: options.listenPort || options.port || 47808,
    ...
    options.reuseAddr = options.reuseAddr == null ? true: options.reuseAddr;
    
    this._transport = this._settings.transport || new baTransport({
      port: this._settings.port,
      listenPort: this._settings.listenPort,
  ...
  _segmentAckResponse(receiver, negative, server, originalInvokeId, sequencenumber, actualWindowSize) {
	...
	this._transport.send(buffer.buffer, buffer.offset, sendAddress);
  }
  ...
  _processUnconfirmedServiceRequest(address, type, service, buffer, offset, length, addressObject) {
    ...
    if (service === baEnum.UnconfirmedServiceChoice.I_AM) {
      ...
      result = baServices.iHaveBroadcast.decode(buffer, offset);
      if (!result) return debug('Received invalid iHave message');

      /**
       * The iAm event represents the response to a whoIs request to detect all devices in a BACNET network.
       * @event bacstack.iHave
       * @param {object} device - An object representing the detected device.
       * @param {string} device.address - The IP address of the detected device.
       * @param {number} device.deviceId - The BACNET device-id of the detected device.
       * @param {number} device.maxApdu - The max APDU size the detected device is supporting.
       ...
       * const client = new bacnet();
       *
       * client.on('iHave', (device) => {
       *   console.log('address: ', device.address, ' - deviceId: ', device.deviceId, ' - objectId: ', device.objectId, ' - objectName: ', device.objectName);
       * });
       */
	    var net = typeof(addressObject)=='object' ? addressObject.net : undefined; 
  	  var adr = typeof(addressObject)=='object' ? addressObject.adr : undefined; 
      this.emit('iHave', {address: address, deviceId: result.deviceId, objectId: result.objectId, objectName: result.objectName, 'net':net, 'adr':adr});
      ...
      switch (result.networkMsgType) {
        case baEnum.NetworkLayerMessageType.I_AM_ROUTER_TO_NETWORK: {
          if (result.data && result.data.networks && result.data.networks.length > 0) {
            this.emit('iAmRouterToNetwork', { address: addressObject, networks: result.data.networks });
          }
          break;
        }
        default: {
  ...
  _receiveData(buffer, remoteAddress) {
    ...
    // Check BVLC function
    if (result.func === baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU || result.func === baEnum.BvlcResultPurpose.ORIGINAL_BROADCAST_NPDU) {
      this._handleNpdu(buffer, result.len, buffer.length - result.len, remoteAddress);
    } else if (result.func === baEnum.BvlcResultPurpose.FORWARDED_NPDU) {
      this._handleNpdu(buffer, result.len, buffer.length - result.len, result.forwardedFor.ip);
    } else {
  ...
  getBBMD() {
    if (this._transport._settings.broadcastAddress === this._settings.broadcastAddress) {
      return null
    } else {
      return this._transport._settings.broadcastAddress
    }
  }
  registerForeignDevice(ip, ttl) {
    if (ip && ttl) {
      if (this._transport._settings.broadcastAddress !== ip) {
        const buffer = this._getBuffer();
        buffer.buffer.writeUIntBE(ttl, buffer.offset, 2);
        buffer.offset += 2;
        const npduType = baEnum.BvlcResultPurpose.REGISTER_FOREIGN_DEVICE; 
        baBvlc.encode(buffer.buffer, npduType, buffer.offset);
        this._transport.send(buffer.buffer, buffer.offset, ip);
        const self = this;
        this._transport._settings.broadcastAddress = ip;
        this._foreignDeviceTimer = setTimeout(() => {
          self.registerForeignDevice(ip, ttl);
        }, ttl * 1000)
      }
    } else {
      this.cancelForeignDevice();
    }
  }

  cancelForeignDevice() {
    this._transport._settings.broadcastAddress = this._settings.broadcastAddress
    if (this._foreignDeviceTimer) {
      clearTimeout(this._foreignDeviceTimer);
      this._foreignDeviceTimer = null;
    }
  }
  ...
  whoIs(options) {
    ...
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, settings.address, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
    ...
    var sendAddress = typeof(settings.address) != 'string' ? settings.address.ip : settings.address;
	//const npduType = (sendAddress !== this._transport.getBroadcastAddress()) ? baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU : baEnum.BvlcResultPurpose.ORIGINAL_BROADCAST_NPDU;
    const npduType = baEnum.BvlcResultPurpose.ORIGINAL_BROADCAST_NPDU; 
	baBvlc.encode(buffer.buffer, npduType, buffer.offset);
	this._transport.send(buffer.buffer, buffer.offset, sendAddress);
  }

  /**
   * The whoHas command discovers BACNET devices in a network with matching criteria.
   * @function bacstack.whoIs
   * @param {object=} options
   * @param {number=} options.lowLimit - Minimal device instance number to search for.
   * @param {number=} options.highLimit - Maximal device instance number to search for.
   * @param {object=} options.objectId - Object id to search for { type: xx, instance: xx } instance 0 is a wildcard
   * @param {string=} options.objectName - Name of object to search for.
   * @param {string=} options.address - Unicast address if command should address a device directly, or address and net
   * @fires bacstack.iHave
   * @example
   * const bacnet = require('bacstack');
   * const client = new bacnet();
   *
   * client.whoHas({ objectId: { type: 8 }, objectName: 'Room123', address: { ip: '255.255.255.255', net: 65535 } });
   */
  whoHas(options) {
    options = options || {};
    const settings = {
    ...
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, settings.address, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
    baApdu.encodeUnconfirmedServiceRequest(buffer, baEnum.PduTypes.UNCONFIRMED_REQUEST, baEnum.UnconfirmedServiceChoice.WHO_HAS);
    baServices.whoHas.encode(buffer, settings.lowLimit, settings.highLimit, settings.objectId, settings.objectName);
    var sendAddress = typeof(settings.address) != 'string' ? settings.address.ip : settings.address;
	//const npduType = (sendAddress !== this._transport.getBroadcastAddress()) ? baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU : baEnum.BvlcResultPurpose.ORIGINAL_BROADCAST_NPDU;
    const npduType = baEnum.BvlcResultPurpose.ORIGINAL_BROADCAST_NPDU; 
	  baBvlc.encode(buffer.buffer, npduType, buffer.offset);
    ...
    /**
   * The whoIsRouterToNetwork command discovers bacnet routers or networks behind routers in a BACNet internetwork.
   * @function bacstack.whoIsRouterToNetwork
   * @param {object=} options
   * @param {number=} options.network - Optional network number
   * @param {string=} options.address - Unicast address if command should address a device directly.
   * @fires bacstack.iAmRouterToNetwork
   * @example
   * const bacnet = require('bacstack');
   * const client = new bacnet();
   *
   * client.whoIsRouterToNetwork({address: '10.0.0.4'});
   */
  whoIsRouterToNetwork(options) {
    options = options || {};
    const settings = {
    ...
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE | baEnum.NpduControlBits.NETWORK_LAYER_MESSAGE, typeof(settings.address) != 'string' ? settings.address.ip : settings.address, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
    var sendAddress = typeof(settings.address) != 'string' ? settings.address.ip : settings.address;
	  const npduType = (sendAddress !== this._transport.getBroadcastAddress()) ? baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU : baEnum.BvlcResultPurpose.ORIGINAL_BROADCAST_NPDU;
    //const npduType = baEnum.BvlcResultPurpose.ORIGINAL_BROADCAST_NPDU; 
	  baBvlc.encode(buffer.buffer, npduType, buffer.offset);
  ...
  readProperty(address, objectId, propertyId, options, next) {
    ...
    if(options.arrayIndex != undefined){
        settings.arrayIndex = options.arrayIndex;
    }
    const buffer = this._getBuffer();
    ...
    baBvlc.encode(buffer.buffer, baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU, buffer.offset);
    var sendAddress = typeof(address) != 'string' ? address.ip : address;
	this._transport.send(buffer.buffer, buffer.offset, sendAddress);
  ...
  deviceCommunicationControl(address, timeDuration, enableDisable, options, next) {
    ...
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE | baEnum.NpduControlBits.EXPECTING_REPLY, address, null, DEFAULT_HOP_COUNT);
  ...
  writeFile(address, objectId, position, fileBuffer, options, next) {
    ...
    // Edit, enforcing 'stream'
    baServices.atomicWriteFile.encode(buffer, true, objectId, position, fileBuffer);
    baBvlc.encode(buffer.buffer, baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU, buffer.offset);
    var sendAddress = typeof(address) != 'string' ? address.ip : address;
    this._transport.send(buffer.buffer, buffer.offset, sendAddress);
    this._addCallback(settings.invokeId, (err, data) => {
  ...
  deleteObject(address, objectId, options, next) {
    ...
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE | baEnum.NpduControlBits.EXPECTING_REPLY, address, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
  ...
  unconfirmedPrivateTransfer(address, vendorId, serviceNumber, data) {
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, address, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
  ...
  readPropertyResponse(receiver, invokeId, objectId, property, value,source) {
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE,receiver,source);
  ...
  readPropertyMultipleResponse(receiver, invokeId, values,source) {
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, receiver,source);
   ...
   /**
   * Responds to both broadcast and unicast I-AM responses 
   * @function bacstack.iAmResponse
   */
  iAmResponse(deviceId, segmentation, vendorId, destAddress,srcAddress) {
    const buffer = this._getBuffer();
    var dAddress = destAddress == undefined ? this._transport.getBroadcastAddress() : destAddress;
    var sAddress = srcAddress == undefined ? this._transport.getBroadcastAddress() : srcAddress;
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, dAddress , sAddress );
    ...
    this._transport.send(buffer.buffer, buffer.offset, dAddress);
  }
  ...
  simpleAckResponse(receiver, service, invokeId) {
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, receiver, null, DEFAULT_HOP_COUNT);
    baApdu.encodeSimpleAck(buffer, baEnum.PduTypes.SIMPLE_ACK, service, invokeId);
    baBvlc.encode(buffer.buffer, baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU, buffer.offset);
    this._transport.send(buffer.buffer, buffer.offset, receiver.ip || receiver.address );
  }

  errorResponse(receiver, service, invokeId, errorClass, errorCode,srcAddress) {
    const buffer = this._getBuffer();
    var sAddress = srcAddress == undefined ? this._transport.getBroadcastAddress() : srcAddress;
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, receiver,sAddress);
