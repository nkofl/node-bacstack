...
class Client extends EventEmitter {
  ...
  registerForeignDevice(ip, ttl, renew = false) {
    if (ip && ttl) {
      if (renew || this._transport._settings.broadcastAddress !== ip) {
        const buffer = this._getBuffer();
        ...
        this._foreignDeviceTimer = setTimeout(() => {
          self.registerForeignDevice(ip, ttl, true);
        }, ttl * 1000)
      }
    } else {
  ...
  timeSync(address, dateTime) {
    const buffer = this._getBuffer();
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, address, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
